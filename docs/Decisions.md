...
- **§R13, found by the verification step: a cache policy and a response headers policy are different things, and `sw.js` needed both.** `Managed-CachingDisabled` stops **CloudFront** caching — confirmed live by `X-Cache: Miss from cloudfront` — but it sets nothing for the **browser**. S3 sends no `Cache-Control` of its own, so the response arrived with none at all and the browser would cache heuristically off `Last-Modified`. Exactly the stale-service-worker failure the behaviour existed to prevent, half-solved. Fixed with an `aws_cloudfront_response_headers_policy` setting `Cache-Control: no-cache`, attached to the `sw.js` and `index.html` behaviours
- **The check that caught it was itself written wrong** — it grepped for a header the config never emitted, so it could only ever have printed nothing. It still did its job: the empty output was the signal. Worth recording because the two failure modes are indistinguishable from the output alone, and the instinct on an empty result is to assume the check is broken rather than the thing under test
...
- **Verified with both halves, and the negative half is the one that counts.** `GET /me` with a valid token returned 200 and the caller's real `sub` and email; with no token it returned **401**. The 200 alone proves nothing about the guard — a route wired without `authorizer_id` returns 200 too, just to anyone. Checking only the happy path is precisely the §B2/§B3 shape: a check that passes for the wrong reason
...
- **Verified end to end, not just applied.** `admin-create-user` → `initiate-auth` returned `ChallengeName: EMAIL_OTP` → a real code arrived → `respond-to-auth-challenge` returned an `AuthenticationResult` with access, id and refresh tokens. `ExpiresIn: 3600` confirms `access_token_validity = 1` with `token_validity_units` actually took effect rather than falling back to the one-day default — a silent fallback there would have gone unnoticed until a token outlived its welcome
